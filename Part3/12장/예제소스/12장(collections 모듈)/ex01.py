...
# 스택, 튜플, 딕셔너리) 좀 더 확장하여 제작된 파이썬의 내장
# 모듈이다.
...
# print(deque_list.pop(0))   # deque 에서는 pop(0)것이 지원이 안된다.
# deque 에서는 스택도 지원하는 모듈이므로 pop() 사용하면 늦게 들어간
# 데이터 부터 삭제됨을 알 수가 있다.
...
print("--------------------------")
# appendleft() 메소드 사용 : 새로운 요소들을 왼쪽부터 입력되게 하여
...
for i in range(5):
    deque_list.appendleft(i)
print(deque_list)
# rotate(n) : 요소들을 n 만큼 회전 해주는 메소드이다.
...
print(deque_list)
basedata = ["a","b","c","d","e"]
# maxlen 매개변수는 deque 의 사이즈를 고정을 시켜버리는 인자값이다.
deque_list = deque(basedata, maxlen=3)
print(deque_list)
# popleft() 메소드는 deque 에서 요소의 왼쪽에 있는 것을 삭제한다.
