...
# 결론은 list, deque 를 성능 테스트를 해보니 데이터를 추가를 할 때는
# deque 가 훨씬 좋은 성능을 발휘한다.또한 대용량일수록 더욱더 많은 차이를
# 나타낸다.중간에 삽입하는 기능 역시 deque 가 훨씬 빠르다라는 것이다.
# 추출할 때는 역시 deque 가 list 보다 훨씬 빠른 성능을 보인다.
# 데이터용량이 크면 클수록 deque 를 사용을 하는 것을 고려해 봐야 한다.