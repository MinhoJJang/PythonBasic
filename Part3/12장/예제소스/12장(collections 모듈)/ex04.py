...
# 정렬된 리스트가 리턴된다.OrderedDict() 로 포장(wrapping) 을 하고있다.
...
# 딕셔너리의 동등성 비교
# 동등성은 논리적 동등이라는 것을 의미한다.논리적 동등이란은 주소는 다르지만
...
print("-------------------------")
from collections import OrderedDict
# OrderedDict 는 두 개의 OrderedDict 를 비교할 때 해당하는 값들의
...
# 결론
# 첫 번째는 OrderedDict 모듈은 딕셔너리의 순서대로 저장하지 않는 방식을
# 순서대로 저장하는 방식으로 개선되었다.(파이썬 버전이 3.6 이후로는 저장과
# 출력이 OrderedDict 와 동일하게 작동을 하고 있지만 2.x 버전에서는 문제점
# 이 발생한 것을 보았다.)
# 두 번째는 딕셔너리의 동등성 비교에서 일반적인 딕셔너리는 순서를 유지하지
# 아도 해당 키와 값이 동등하다면 True 를 리턴하지만, OrderedDict 에서는
# 순서, 키와 값이 무조건 같아야 True 를 리턴하는 좀 더 깐깐한 동등성 비교로
# 개선되었다.
# OrderedDict 를 사용을 하면 정확한 데이터의 순서나 값을 유지하는데 일반
# 딕셔너리에 비해서는 엄청 좋은 면이 존재한다.
# 웬만하면 딕셔너리보다는 OrderedDict 모듈을 이용하는 것이 현명하지 않은가
# 라는 생각을 해봅니다.
