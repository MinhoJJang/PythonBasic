...
# 자료구조의 특징 : 최대한 메모리를 효율적으로 저장 및 반환하는 방법으로, 데이터를
# 관리하는 것, 대용량일수록 빨리 저장하고 빨리 검색하고, 메모리를 최대한 효율적으로
...
# pop() 메서드는 스택에서 마지막으로 들어온 데이터를 삭제하면서 삭제된 데이터를
# 반환한다.
...
result = []
# _ 라는 기호는 파이썬에서 상당히 많이 사용되는 기호인데, for 문을 루핑시킬 때, 루프
# 변수에 값을 사용하지 않을때, _ 로 할당받는 것이다.
