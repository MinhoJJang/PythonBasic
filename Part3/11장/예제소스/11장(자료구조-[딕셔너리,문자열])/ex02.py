...
# 아래와 같이 루프를 돌리면 키와 값을 동시에 출력이 된다.(튜플 형태)
for subject in scores.items():
    print(subject, end=" ")
print()

print("---------------------------")
# 딕셔너리 함축(dictionary comprehension)방법은 코드를 간결하게 만들고
# 가독성도 좋아진다.
...
# 파이썬에서 딕셔너리는 근본적으로 요소들이 순서가 없기 때문에 순서대로
# 저장하지 않는다.
...
print(li2)
print("---------------------------")
# 딕셔너리의 값을 정렬하고자 한다면 values()함수와 sorted()를 같이 사용하면
# 된다
...
print("---------------------------")
# 딕셔너리의 값에 따라서 키들을 정렬하고 싶은 경우에는 sorted()에 요소들
# 을 비교할 때 사용하는 키를 지정해주면 된다.
